...
we'll define a simple kinematic chain based on the following [URDF](http://wiki.ros.org/urdf/XML/model) and then using
that model we'll implement an algorithm for calculating the inverse dynamics of a kinematic chain.
...
Recursive Newton Euler Algorithm for inverse dynamics, it takes a bit of time to figure out the twist, acceleration and
force calculations but other than that it's not too hard to follow so by all means take a look at the implementation in
InverseDynamics.cpp
...
void setup()
{
    ...
    Serial << "Dangling:\n";
    Serial << "shoulder torque: " << shoulder.torque << "\n";
    Serial << "elbow torque: " << elbow.torque << "\n";
    Serial << "wrist torque: " << wrist.torque << "\n\n";
