 ...
 * with them, then have a read through https://en.wikipedia.org/wiki/Rotation_matrix 
 */
...
void setup() 
{
  ...
  // You can rotate a rotation matrix about the X, Y or Z axis using the Rotate methods. Note that all angles should be specified in radians.
  R1.RotateZ(M_PI_4);
  R1.RotateY(0.9);
  R1.RotateX(0.2);
  ...
  // You can think of a rotation matrix as just three 3x1 vectors which describe the direction of the x/y/z axes of the rotated frame in base frame coordinates.
  ...
  // Now looking at the magnitudes we can see that they're unit vectors
  ...
  // So these opearations are only analogous if the R3 matrix example isn't already rotated.
  ...
  // The order of rotations euler angles goes: X, Y, Z; meaning that R1 and R3 are equivilent:
  ...
  Serial << "R2 as euler angles: " << R3.ToEulerAngles()  << "\n\n";
  ...
  // Likewise you can rotate rotations by multiplying them together. Formally speaking, to apply a rotation represented by say R1 to R2 you must premultiply R2 by R1 like so:
  Rotation R4 = R2 * R1;
  ...
  // is a coordinate frame whose origin is located at a particular point. They're useful for representing a position plus an orientation which might come up when you're programming your quadrocopter,
  // controlling the position of the end effector of your robot arm and so on.
  Transformation T1, T2;

  // You can access the origin (Point) of the transformtion directly through it's member T.p and you can access the rotation of the frame through T.R. These are just instances of Point and Rotation respectively so everything
  // we've covered so far still applies to them:
  T1.p.X() = 23.54;
  ...
  // And of course you can print them to Serial in the usual way, note that it'll print out just like a 4x4 matrix
  Serial << "T3 = " <<  T3 << "\n";
